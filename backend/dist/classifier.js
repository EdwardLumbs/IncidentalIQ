...
let cachedBatchSystemPrompt = null;
function buildBatchSystemPrompt() {
    if (cachedBatchSystemPrompt !== null)
...
function buildBatchUserPrompt(args) {
    ...
    for (const m of messages)
        out += `[${m.id}] ${m.sender ?? "unknown"}: ${m.content}\n`;
    return out;
}
...
export async function classifyBatch(args) {
    ...
    if (!messages?.length)
        return { out: { results: [], situation_summary: summary ?? "" }, meta: null };
    ...
    const normId = (v) => String(v ?? "").replace(/^\s*\[|\]\s*$/g, "").trim();
    ...
    const results = raw
        ...
        incidentals: Array.isArray(r.incidentals)
            ? r.incidentals.filter((x) => x && TYPE_KEYS.includes(x.incidental_type)).map((x) => ({
    ...
    const situation_summary = typeof parsed.situation_summary === "string" && parsed.situation_summary.trim()
...
export async function contentHash(source, group, message) {
...
export const normContainer = (s) => s.toUpperCase().replace(/\s+/g, "");
