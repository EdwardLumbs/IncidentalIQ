...
import { classifyBatch, CONTAINER_RE, PLATE_RE, normContainer, } from "./classifier.js";
import { insertMessages, chatsWithUnclassified, unclassifiedFor, getSummary, getTrips, saveSummary, saveChunkResults, batchUpsertTrips, queryIncidentals, queryIncidentalsByTrip, queryMessages, queryTrips, setSystemStatus, getSystemStatus, bumpMetrics, getTripLinks, upsertTripLinks, applyTripLinks, nowPh, } from "./db.js";
// Hard limits.
...
function listResponse(res, rows, key, format) {
    if (format === "csv") {
...
app.use((req, res, next) => {
    if (AUTH_EXEMPT.has(req.path) || authorized(req))
        return next();
...
app.post("/messages", asyncHandler(async (req, res) => {
    ...
    if (!msgs)
        return res.status(400).json({ ok: false, error: "expected { messages: [...] }" });
...
app.get("/incidentals", asyncHandler(async (req, res) => {
    const p = req.query;
...
function stampFor(message, links) {
...
async function runClassifier() {
    ...
    for (const group of chats) {
        ...
        for (let i = 0; i < msgs.length; i += batchSize) {
            ...
            const tripStamp = new Map();
            for (const m of chunk) {
                const stamp = stampFor(m.message, links);
                if (stamp)
                    tripStamp.set(m.id, stamp);
            }
            ...
            for (const r of chunkResults) {
                if (r.trip_reference)
                    tripRefs.push({ ref: r.trip_reference, type: r.reference_type });
            }
            for (const m of chunk) {
...
async function main() {
    await initSchema(async (sql) => pgRun(sql));
