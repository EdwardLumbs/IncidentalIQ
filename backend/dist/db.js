...
const dedupWindowFor = (message) => message.trim().length >= SUBSTANTIVE_LEN ? DEDUP_WINDOW_LONG_MS : DEDUP_WINDOW_SHORT_MS;
function normalizeSource(s) {
    const v = (s || "").toLowerCase();
    if (v.includes("viber"))
        return "viber";
    if (v.includes("messenger") || v.includes("orca"))
        return "messenger";
    return v || "unknown";
}
...
export async function insertMessages(msgs) {
    ...
    for (const m of msgs) {
        ...
        if (!isFinite(tsMs) || tsMs > nowMs + 3_600_000 || tsMs < nowMs - 2 * 86_400_000)
            tsMs = nowMs;
    ...
    const seenTimes = new Map(); // hash → [tsMs, ...] already present
    const lookupRows = await all(`SELECT content_hash, timestamp FROM incidentaliq.captured_messages
      WHERE timestamp >= $1 AND content_hash = ANY($2::text[])`, [loIso, uniqueHashes]);
    ...
    for (const p of prepared) {
        ...
        if (times.some((t) => Math.abs(t - p.tsMs) < win)) {
            skipped++;
            continue;
        }
        fresh.push(p);
    ...
    await run(`INSERT INTO incidentaliq.captured_messages
      (source, group_name, sender, message, image_url, timestamp, raw_notif, content_hash, synced_at, store_only)
     VALUES ${rowsSql}`, binds);
    inserted = fresh.length;
    return { inserted, skipped };
}
// Distinct chats that have unclassified messages waiting.
export async function chatsWithUnclassified() {
    const rows = await all("SELECT DISTINCT group_name FROM incidentaliq.captured_messages WHERE classified_at IS NULL AND store_only = 0");
...
export async function unclassifiedFor(group, limit = 500) {
    return all(`SELECT id, sender, message, timestamp FROM incidentaliq.captured_messages
     WHERE group_name = $1 AND classified_at IS NULL AND store_only = 0 ORDER BY timestamp ASC, id ASC LIMIT $2`, [group, limit]);
}
export async function getSummary(group) {
    const row = await get("SELECT situation_summary FROM incidentaliq.chat_state WHERE group_name = $1", [group]);
    return row?.situation_summary ?? null;
}
// Recent trips for a chat (registry fed into the prompt). Capped + freshness-filtered.
export async function getTrips(group, limit = 40, staleDays = 21) {
    const cutoff = phFromEpoch(Date.now() - staleDays * 86400_000);
    return all(`SELECT trip_reference, reference_type, driver, helper FROM incidentaliq.trips
     WHERE group_name = $1 AND (last_seen IS NULL OR last_seen >= $2)
     ORDER BY last_seen DESC LIMIT $3`, [group, cutoff, limit]);
}
export async function saveSummary(group, summary) {
    await run(`INSERT INTO incidentaliq.chat_state (group_name, situation_summary, updated_at) VALUES ($1, $2, $3)
     ON CONFLICT (group_name) DO UPDATE SET situation_summary = $2, updated_at = $3`, [group, summary, nowPh()]);
}
// Upsert many trips in one transaction.
export async function batchUpsertTrips(group, refs, seenAt) {
    const seen = new Set();
    ...
    await tx(async (t) => {
        for (const { ref, type } of rows) {
            await t.run(`INSERT INTO incidentaliq.trips (group_name, trip_reference, reference_type, first_seen, last_seen)
         VALUES ($1, $2, $3, $4, $4)
         ON CONFLICT (group_name, trip_reference) DO UPDATE SET
           last_seen = $4,
           reference_type = COALESCE(incidentaliq.trips.reference_type, $3)`, [group, ref, type, seenAt]);
        }
    });
}
...
export async function getTripLinks(group) {
    const rows = await all("SELECT alias, canonical FROM incidentaliq.trip_links WHERE group_name = $1", [group]);
    const m = new Map();
    for (const r of rows)
        m.set(r.alias, r.canonical);
    return m;
}
// Remember new alias→canonical pairs (idempotent).
export async function upsertTripLinks(group, pairs) {
    const now = nowPh();
    const seen = new Set();
    const rows = pairs.filter(({ alias, canonical }) => {
        if (!alias || !canonical || alias === canonical || seen.has(alias))
            return false;
        seen.add(alias);
        return true;
    });
    ...
    await tx(async (t) => {
        for (const { alias, canonical } of rows) {
            await t.run(`INSERT INTO incidentaliq.trip_links (group_name, alias, canonical, updated_at) VALUES ($1, $2, $3, $4)
         ON CONFLICT (group_name, alias) DO UPDATE SET canonical = $3, updated_at = $4`, [group, alias, canonical, now]);
        }
    });
}
...
export async function applyTripLinks(group, pairs) {
    const seen = new Set();
    const rows = pairs.filter(({ alias, canonical }) => {
        if (!alias || !canonical || alias === canonical || seen.has(alias))
            return false;
    ...
    await tx(async (t) => {
        for (const { alias, canonical } of rows) {
            await t.run(`UPDATE incidentaliq.captured_messages SET trip_reference = $2, reference_type = 'container_number'
         WHERE group_name = $1 AND trip_reference = $3`, [group, canonical, alias]);
            await t.run(`UPDATE incidentaliq.incidentals SET trip_reference = $2
         WHERE trip_reference = $1 AND message_id IN (SELECT id FROM incidentaliq.captured_messages WHERE group_name = $3)`, [alias, canonical, group]);
        }
    });
}
...
export async function saveChunkResults(chunkIds, results, tripStamp) {
    const now = nowPh();
    ...
    const canonTrip = (ref, type) => ref && type === "plate_number" ? ref.toUpperCase() : ref;
    await tx(async (t) => {
        for (const id of chunkIds) {
            ...
            if (!r) {
                ...
                if (stamp) {
                    await t.run(`UPDATE incidentaliq.captured_messages SET classified_at = $2, is_incidental = 0,
               trip_reference = $3, reference_type = $4, reference_source = 'regex' WHERE id = $1`, [id, now, stamp.ref, stamp.type]);
                }
                else {
                    await t.run("UPDATE incidentaliq.captured_messages SET classified_at = $2, is_incidental = 0 WHERE id = $1", [id, now]);
                }
                continue;
            }
            ...
            await t.run(`UPDATE incidentaliq.captured_messages SET classified_at = $2, is_incidental = 1,
           trip_reference = $3, reference_type = $4, reference_source = $5 WHERE id = $1`, [id, now, trip, refType, refSource]);
            for (const inc of r.incidentals) {
                await t.run(`INSERT INTO incidentaliq.incidentals
             (message_id, incidental_type, status, confidence, amount, trip_reference)
           VALUES ($1, $2, $3, $4, NULL, $5)
           ON CONFLICT (message_id, incidental_type, status) DO NOTHING`, [id, inc.incidental_type, inc.status, inc.confidence, trip]);
                incidentalCount++;
            }
        }
    });
...
export async function setSystemStatus(key, value) {
    await run(`INSERT INTO incidentaliq.system_status (key, value, updated_at) VALUES ($1, $2, $3)
     ON CONFLICT (key) DO UPDATE SET value = $2, updated_at = $3`, [key, value, nowPh()]);
}
export async function getSystemStatus() {
    const rows = await all("SELECT key, value, updated_at FROM incidentaliq.system_status");
    const out = {};
    for (const r of rows)
...
export async function bumpMetrics(delta) {
    const row = await get("SELECT value FROM incidentaliq.system_status WHERE key = 'metrics'");
    const m = {
        cron_runs: 0, total_batches: 0, total_tokens: 0, total_incidentals: 0, total_processed: 0, last_run_at: "",
    };
    if (row?.value) {
        try {
            Object.assign(m, JSON.parse(row.value));
        }
        catch { }
    }
    m.cron_runs += 1;
...
function normDate(s, endOfDay = false) {
    if (/^\d{4}-\d{2}-\d{2}$/.test(s))
        return s + (endOfDay ? "T23:59:59+08:00" : "T00:00:00+08:00");
    return toPh(s);
}
...
const clampLimit = (n) => Math.min(Math.max(n ?? 200, 1), 10000);
const clampOffset = (n) => Math.max(n ?? 0, 0);
// GET /incidentals — join incidentals to their message, newest first, optional filters + paging.
export async function queryIncidentals(opts) {
    const clauses = [];
    const binds = [];
    if (opts.trip) {
        binds.push(opts.trip);
        clauses.push(`i.trip_reference = $${binds.length}`);
    }
    if (opts.status) {
        binds.push(opts.status);
        clauses.push(`i.status = $${binds.length}`);
    }
    if (opts.group) {
        binds.push(`%${opts.group}%`);
        clauses.push(`m.group_name ILIKE $${binds.length}`);
    }
    if (opts.since) {
        binds.push(normDate(opts.since));
        clauses.push(`m.timestamp >= $${binds.length}`);
    }
    if (opts.until) {
        binds.push(normDate(opts.until, true));
        clauses.push(`m.timestamp <= $${binds.length}`);
    }
    const where = clauses.length ? `WHERE ${clauses.join(" AND ")}` : "";
    binds.push(clampLimit(opts.limit));
    const limIdx = binds.length;
    binds.push(clampOffset(opts.offset));
    const offIdx = binds.length;
    return all(`SELECT i.id, i.incidental_type, i.status, i.confidence, i.trip_reference,
     ...
     ${where}
     ORDER BY m.timestamp DESC LIMIT $${limIdx} OFFSET $${offIdx}`, binds);
}
...
export async function queryIncidentalsByTrip(opts) {
    const clauses = [];
    const binds = [];
    if (opts.status) {
        binds.push(opts.status);
    ...
    const where = clauses.length ? `WHERE ${clauses.join(" AND ")}` : "";
    return all(`SELECT COALESCE(i.trip_reference, '(unlinked)') AS trip_reference,
            m.group_name,
     ...
     ORDER BY incidentals DESC, last_seen DESC`, binds);
}
...
export async function queryMessages(opts) {
    ...
    if (opts.trip) {
        binds.push(opts.trip.toUpperCase());
        clauses.push(`UPPER(m.trip_reference) = $${binds.length}`);
    }
    if (opts.group) {
        binds.push(`%${opts.group}%`);
        clauses.push(`m.group_name ILIKE $${binds.length}`);
    ...
    if (opts.incidental === "1" || opts.incidental === "true") {
        clauses.push(`m.is_incidental = 1`);
    }
    // store_only filter: =1 pulls ONLY archive-only groups, =0 excludes them; omit for everything.
    if (opts.store_only === "1" || opts.store_only === "true") {
        clauses.push(`m.store_only = 1`);
    }
    else if (opts.store_only === "0" || opts.store_only === "false") {
        clauses.push(`m.store_only = 0`);
    }
    const where = clauses.length ? `WHERE ${clauses.join(" AND ")}` : "";
    binds.push(clampLimit(opts.limit));
    const limIdx = binds.length;
    ...
    const rows = await all(`SELECT m.id, m.source, m.group_name, m.sender, m.message, m.timestamp,
...
export async function queryTrips(opts) {
    const clauses = [];
    const binds = [];
    if (opts.group) {
        binds.push(`%${opts.group}%`);
        clauses.push(`t.group_name ILIKE $${binds.length}`);
    }
    if (opts.since) {
        binds.push(normDate(opts.since));
        clauses.push(`t.last_seen >= $${binds.length}`);
    }
    if (opts.until) {
        binds.push(normDate(opts.until, true));
        clauses.push(`t.last_seen <= $${binds.length}`);
    ...
    return all(`SELECT t.group_name, t.trip_reference, t.reference_type, t.driver, t.helper,
     ...
     ${where}
     ORDER BY t.last_seen DESC LIMIT $${limIdx} OFFSET $${offIdx}`, binds);
}
